        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <SettingsLayout>
                <div className="space-y-6">
                    <HeadingSmall title={__('profile.profile_information')} description={__('profile.profile_information_desc')} />
                    ...
                    >
                        {({ processing, errors }) => (
                            <>
                                ...
                                <div className="grid gap-2">
                                    <Label htmlFor="email">{__('profile.email')}</Label>
                                ...
                                <div className="grid gap-2">
                                    <Label htmlFor="locale">{__('profile.language')}</Label>
                                    ...
                                    <Select value={locale} onValueChange={setLocale}>
                                        ...
                                        <SelectContent>
                                            <SelectItem value="en">{__('profile.english')}</SelectItem>
                                            <SelectItem value="sr">{__('profile.serbian')}</SelectItem>
                                        </SelectContent>
                                    </Select>
                                ...
                                <div className="grid gap-2">
                                    <Label htmlFor="password_confirm_minutes">{__('profile.require_password_after_inactivity')}</Label>
                                    ...
                                    <Select value={passwordConfirmMinutes} onValueChange={setPasswordConfirmMinutes}>
                                        ...
                                        <SelectContent>
                                            <SelectItem value="0">{__('profile.inactivity_off')}</SelectItem>
                                            <SelectItem value="30">{__('profile.inactivity_30')}</SelectItem>
                                            <SelectItem value="60">{__('profile.inactivity_60')}</SelectItem>
                                            <SelectItem value="240">{__('profile.inactivity_240')}</SelectItem>
                                            <SelectItem value="600">{__('profile.inactivity_600')}</SelectItem>
                                        </SelectContent>
                                    </Select>
                                    <p className="text-xs text-muted-foreground">{__('profile.require_password_hint')}</p>
                ...
                <div className="rounded-lg border p-4">
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            <p className="font-medium">{__('profile.two_factor')}</p>
                            <p className="text-sm text-muted-foreground">{__('profile.two_factor_desc')}</p>
                            <div className="mt-1">
                                {enabled ? (
                                    <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2 py-0.5 text-xs font-medium text-emerald-700 dark:text-emerald-400">
                                        <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" aria-hidden="true"></span>
                                        {__('profile.status_enabled')}{tfType ? ` (${String(tfType).toUpperCase()})` : ''}
                                    </span>
                                    ...
                                    <span className="inline-flex items-center gap-1 rounded-full bg-amber-500/10 px-2 py-0.5 text-xs font-medium text-amber-700 dark:text-amber-400">
                                        ...
                                        {__('profile.status_setting_up')}{tfType ? ` (${String(tfType).toUpperCase()})` : ''}
                                    </span>
                                ) : (
                ...
                <div className="rounded-lg border p-4">
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            <p className="font-medium">{__('profile.sessions_card_title')}</p>
                            <p className="text-sm text-muted-foreground">{__('profile.sessions_card_desc')}</p>
                        </div>
